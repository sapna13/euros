...
public final class Script implements Serializable {
    ...
    public static Script getByAlpha4Code(String alpha4code) {
        final Script s = byAlpha4.get(Character.toUpperCase(alpha4code.charAt(0)) + alpha4code.substring(1).toLowerCase());
        if(s == null) {
        	throw new LanguageCodeFormatException("Bad script code " + alpha4code);
        }
        return s;
    }
    ...
    public static Script getByNumericCode(int numericCode) {
        final Script s = byNumeric.get(numericCode);
        if(s == null) {
        	throw new LanguageCodeFormatException("Bad script code " + numericCode);
        }
        return s;
